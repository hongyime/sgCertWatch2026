  ...
  for (const brand of data?.watchlist?.brands || []) {
    ...
    for (let ti = 0; ti < brandTokens.length; ti++) {
      ...
      if (exact) {
        ...
        const floorEligible = strongContext && (tokenCompact.length >= 4 || sgNexus || deceptionMarker);
        if (floorEligible) {
        ...
        if (effectiveMaxDist > 0) {
          for (const label of edLabels) {
            ...
            if (brand.id === "carousell" && label.includes("carousel") && !label.includes("carousell") && !hasExplicitContext) continue;
      ...
      if (segExact) {
        ...
        for (let L = lo; L <= hi && !tokenSquat; L++) {
          ...
          for (const seg of bucket) {
            if (brand.id === "carousell" && seg.includes("carousel") && !seg.includes("carousell") && !hasExplicitContext) continue;
