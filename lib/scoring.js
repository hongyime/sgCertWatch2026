...
function minSubstringEditDistance(label, token, maxDistance) {
  ...
  for (let w = minWindow; w <= maxWindow; w++) {
    for (let i = 0; i <= labelLen - w; i++) {
      ...
      if (Math.abs(windowStr.length - tokenLen) <= maxDistance) {
...
function brandSignals(domain, data, passedHost = null, passedRegistrable = null) {
  ...
  const subCandidateLabelsWithAffix = combinedSubLabels.flatMap((l) => decomposeLabel(l, affixKeywords));

  for (const brand of data?.watchlist?.brands || []) {
    const contextHit = !brand.require_context
      || brand.context_tokens?.some((token) => hostCompact.includes(compact(token)) || skeletonCompact.includes(compact(token)));
    ...
    const candidateLabels = brand.allow_affix ? candidateLabelsWithAffix : combinedLabels;
    const subCandidateLabels = brand.allow_affix ? subCandidateLabelsWithAffix : combinedSubLabels;
      ...
      if (exact) {
        ...
        if (effectiveMaxDist > 0) {
          for (const label of candidateLabels) {
            ...
            const dist = minSubstringEditDistance(label, tokenCompact, effectiveMaxDist);
            if (dist > 0) {
