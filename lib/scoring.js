...
function brandSignals(domain, data, passedHost = null, passedRegistrable = null) {
  ...
  for (const brand of data?.watchlist?.brands || []) {
    const contextHit = !brand.require_context
      || brand.context_tokens?.some((token) => {
      ...
      || (brand.tokens || []).some((token) => rawHostSegments.has(compact(token)))
        && keywordTokens.some((k) => hostCompact.includes(k));
    ...
    for (const token of brand.tokens || []) {
      ...
      let exact = candidateLabels.includes(tokenCompact)
        || combinedSubLabels.some((l) => l === tokenCompact);
      ...
      if (exact) {
        ...
        if (effectiveMaxDist > 0) {
          for (const label of candidateLabels) {
            ...
            // Shared generic-morpheme stem comparison: pochtabank vs maybank must compare
            // stems (pocht vs may), not let the shared -bank suffix absorb the edit budget.
