...
function brandSignals(domain, data, passedHost = null, passedRegistrable = null) {
  ...
  const candidateLabelsWithAffix = allLabels.flatMap((l) => decomposeLabel(l, affixKeywords));
  const subCandidateLabelsWithAffix = subLabels.flatMap((l) => decomposeLabel(l, affixKeywords));
  ...
  for (const brand of data?.watchlist?.brands || []) {
    ...
    const candidateLabels = brand.allow_affix ? candidateLabelsWithAffix : allLabels;
    const subCandidateLabels = brand.allow_affix ? subCandidateLabelsWithAffix : subLabels;
