...
function brandSignals(domain, data, passedHost = null, passedRegistrable = null) {
  ...
  for (const brand of data?.watchlist?.brands || []) {
    ...
    for (let ti = 0; ti < brandTokens.length; ti++) {
      ...
      // Anchor eligibility: fuzzy/squat signals may only lift a finding to alert grade when
      // backed by real context, or by Singapore nexus plus structural corroboration.
      const apexFuzzySelf = !apexSelf && registrableCore.length >= 3
        && Math.abs(registrableCore.length - tokenCompact.length) <= 1
        && editDistanceAtMost(registrableCore, tokenCompact, 1);
      const anchorOK = ctxViaToken || deceptionMarker || prefixCompound
        || (sgNexus && (hostHasKeyword || segExact || apexFuzzySelf));
      ...
      if (matchType) {
        ...
        signals.push({
          ...
          distance: matchDist,
          points,
          nonAnchoring: !anchorOK || matchType === "brand:edit_distance_2"
        });
      }
  ...
  if (idn.isPunycode || (idn.skeleton && idn.skeleton !== host)) {
    ...
    for (const brand of data?.watchlist?.brands || []) {
      for (const token of brand.tokens || []) {
        ...
        if (!hit) continue;
        const foreignPunycode = !sgNexusSuffix;
        if (idn.isPunycode) {
          signals.push({
            ...
            nonAnchoring: foreignPunycode && !idn.hasMixedScript
          });
        }
