...
function brandSignals(domain, data, passedHost = null, passedRegistrable = null) {
  ...
  const hostLabelCountRaw = host.split(".").length;
  const suffixLabelCountRaw = registrable ? suffixOfRegistrable(registrable).split(".").length : 0;
  const coreDotLabels = host
    .split(".")
    .slice(0, Math.max(0, hostLabelCountRaw - suffixLabelCountRaw))
    .map(compact);
  const skelLabels = idn.skeleton && idn.skeleton !== host
    ? idn.skeleton.split(".").slice(0, Math.max(0, hostLabelCountRaw - suffixLabelCountRaw)).map(compact)
    : [];
  const leetLabels = coreDotLabels.flatMap(leetCached).map(compact);
  const combinedLabels = [...new Set([...coreDotLabels, ...skelLabels, ...leetLabels])].filter(Boolean);
  ...
  const edLabels = [...new Set([
    ...combinedLabels,
    ...rawHostSegments
  ])];
  const SEGMENTS_BY_LEN = new Map();
  for (const seg of rawHostSegments) {
    const arr = SEGMENTS_BY_LEN.get(seg.length);
    if (arr) arr.push(seg); else SEGMENTS_BY_LEN.set(seg.length, [seg]);
  }

  const hostMask = charMask(hostCompact);
  const skelMask = idn.skeleton ? charMask(compact(idn.skeleton)) : null;
  if (skelMask) {
    hostMask.l |= skelMask.l;
    hostMask.d |= skelMask.d;
  }
  const sgNexus = /(^|\.)sg$/.test(tld) || tld === "sg";
  const hostHasKeyword = keywordTokens.some((k) => hostCompact.includes(k));
  for (const brand of data?.watchlist?.brands || []) {
    ...
    for (let i = 0; i < brandTokens.length; i++) {
      ...
      if (!mask || popcount32(mask.l & ~hostMask.l) + popcount32(mask.d & ~hostMask.d) <= 1) {
        tokenPossible = true;
        break;
      }
    }
    ...
    const ctxViaToken = !brand.require_context
    ...
    // B2 Batch F2: presence of an actual context token in the host. Deliberately does NOT
    // include the '!require_context' short-circuit - for require_context:false brands that
    // term is constant-true and would let every fuzzy match anchor on zero evidence.
    const contextTokenPresent = (brand._compactContextTokens || (brand.context_tokens || []).map(compact)).some((tokenCompact) => {
      if (rawHostSegments.has(tokenCompact)) return true;
      return tokenCompact.length >= 4 && (hostCompact.includes(tokenCompact) || skeletonCompact.includes(tokenCompact));
    });
    const ctxViaSgSuffix = sgNexus
      && (brand._compactContextTokens || []).includes("sg");
    const deceptionMarker = Boolean(idn.skeleton) && idn.skeleton !== host;
    const contextHit = ctxViaToken
      || ctxViaSgSuffix
      || deceptionMarker
      || (brandTokens.some((t) => rawHostSegments.has(t)) && hostHasKeyword);
    if (!contextHit) continue;
    const strongContext = ctxViaToken || ctxViaSgSuffix || hostHasKeyword || deceptionMarker;
    ...
    for (let ti = 0; ti < brandTokens.length; ti++) {
      ...
      const tokenCharsPresent = !tMask || maskSubset(tMask, hostMask);
      let exact = tokenCharsPresent && (
        candidateSet.has(tokenCompact)
        || combinedSubSet.has(tokenCompact)
        || rawHostSegments.has(tokenCompact));
      let prefixCompound = false;
      if (!exact) {
        // Prefix-compound: brand token opens a longer segment whose tail carries a keyword,
        // generic morpheme, or affix marker (ubogroup, maribankauthaccess, cpff-online).
        for (const label of rawHostSegments) {
          if (!label.startsWith(tokenCompact) || label === tokenCompact) continue;
          const rest = label.slice(tokenCompact.length);
          const restSignalled = GENERIC_MORPHEMS.some((m) => rest === m)
            || affixKeywords.some((a) => rest === a)
            || keywordTokens.some((k) => rest.includes(k));
          if (restSignalled) { exact = true; prefixCompound = true; break; }
        }
      }
      ...
      // G-4 (exact-requires-ctx): tokens that ARE themselves common words need a real
      // context-token hit even for exact matching; absence of context is not evidence.
      if (exact && brand._exactRequiresCtxTokens?.has(tokenCompact) && !contextTokenPresent && !deceptionMarker) {
        exact = false;
      }
      let matchType = null;
      let matchDist = 0;
      ...
      if (exact) {
        ...
        // G-1: token collides with a common word → force exact-match only (effectiveMaxDist=0).
        let floorEligible = false;
        if (brand._forcedExactTokens?.has(tokenCompact)) {
          effectiveMaxDist = 0;
        } else {
          floorEligible = strongContext && (tokenCompact.length >= 4 || sgNexus || deceptionMarker);
          if (floorEligible) {
            effectiveMaxDist = Math.max(effectiveMaxDist, 1);
          }
        }
        if (effectiveMaxDist > 0) {
          for (const label of edLabels) {
            ...
            if (cmpToken !== tokenCompact && !floorEligible) {
      ...
      const subEffectiveMaxDist = brand._forcedExactTokens?.has(tokenCompact) ? 0 : Math.min(
        1,
      ...
      let segExact = rawHostSegments.has(tokenCompact);
      if (!segExact) {
        for (const seg of rawHostSegments) {
          if (leetCached(seg).some((v) => v === tokenCompact)) { segExact = true; break; }
        }
      }
      let tokenSquat = false;
      if (segExact) {
        const commonWordNeedsContext = brand._exactRequiresCtxTokens?.has(tokenCompact)
          && !contextTokenPresent
          && !deceptionMarker;
        tokenSquat = !apexBenign && !commonWordNeedsContext;
      } else if (subEffectiveMaxDist > 0) {
        const lo = tokenCompact.length - subEffectiveMaxDist;
        const hi = tokenCompact.length + subEffectiveMaxDist;
        for (let L = lo; L <= hi && !tokenSquat; L++) {
          const bucket = SEGMENTS_BY_LEN.get(L);
          if (!bucket) continue;
          for (const seg of bucket) {
            if (editDistanceAtMost(seg, tokenCompact, subEffectiveMaxDist)) {
              tokenSquat = true;
              break;
            }
          }
      ...
      subSquat = subSquat || tokenSquat || prefixCompound;

      if (!matchType && !subSquat) continue;

      // Anchor predicate (B2 Batch F): anchoring describes the structural quality of the
      // match, never geography.
      //   - brand:exact on a distinct label segment ......... always anchors
      //   - subdomain_brand_squat on an exact segment ........ always anchors
      //   - fuzzy (edit-distance window or squat-fuzz) ....... anchors only with a
      //     context token hit or a deception marker
      const fuzzyAnchorOK = contextTokenPresent || ctxViaSgSuffix || deceptionMarker;
      const fuzzyMatch = matchType === "brand:edit_distance_1" || matchType === "brand:edit_distance_2";
      if (matchType) {
        ...
        signals.push({
          ...
          distance: matchDist,
          points,
          nonAnchoring: fuzzyMatch && !fuzzyAnchorOK
        });
      }
      if (!isBrandAllowlisted && subSquat) {
        signals.push({
          ...
          nonAnchoring: !segExact && !fuzzyAnchorOK
        });
      }
...
function homoglyphSignals(domain, data, passedHost = null, passedRegistrable = null) {
  ...
  // Match = token equals a whole skeleton segment, or sits within edit distance 1 of one
  // for tokens of length >= 4. Bare substring containment let two-letter tokens ('sc')
  // match anywhere inside transformed skeletons.
  const sgNexusSuffix = /(^|\.)sg$/.test(host.split(".").pop() || "");
  if (idn.isPunycode || (idn.skeleton && idn.skeleton !== host)) {
    const skelSegments = new Set(
      idn.skeleton.split(".").flatMap((p) => p.split(/[-_]/)).map(compact).filter(Boolean)
    );
    for (const brand of data?.watchlist?.brands || []) {
      for (const token of brand.tokens || []) {
        const tokenCompact = compact(token);
        if (tokenCompact.length < 3) continue;
        let hit = skelSegments.has(tokenCompact) && skeletonCompact.includes(tokenCompact);
        if (!hit && tokenCompact.length >= 4) {
          for (const seg of skelSegments) {
            if (Math.abs(seg.length - tokenCompact.length) <= 1 && editDistanceAtMost(seg, tokenCompact, 1)) {
              hit = true;
              break;
            }
          }
...
function scoreDomain(domain, data, now = new Date(), domainMetadata = null) {
  ...
  const hasAnchor = signals.some((s) => anchorTypes.includes(s.type) && s.nonAnchoring !== true);
...
function scoreCertificate(entry, data, now = new Date()) {
  ...
  const hasAnchor = signals.some((s) => anchorSignals.includes(s.type) && s.nonAnchoring !== true);
  const noAnchorCap = cfg.thresholds?.no_anchor_score_cap;
