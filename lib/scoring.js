...
const DP_BUFFER = new Int32Array(256 * 256);
...
function editDistanceAtMost(a, b, maxDistance) {
  ...
  const la = a.length;
  const lb = b.length;
  if (la >= 255 || lb >= 255) return false;

  const stride = lb + 1;
  for (let i = 0; i <= la; i++) DP_BUFFER[i * stride] = i;
  for (let j = 0; j <= lb; j++) DP_BUFFER[j] = j;

  for (let i = 1; i <= la; i++) {
    let rowMin = DP_BUFFER[i * stride];
    const prevRow = (i - 1) * stride;
    const currRow = i * stride;
    for (let j = 1; j <= lb; j++) {
      const cost = a[i - 1] === b[j - 1] ? 0 : 1;
      let val = Math.min(
        DP_BUFFER[prevRow + j] + 1,       // deletion
        DP_BUFFER[currRow + j - 1] + 1,   // insertion
        DP_BUFFER[prevRow + j - 1] + cost // substitution
      );
      if (i > 1 && j > 1 && a[i - 1] === b[j - 2] && a[i - 2] === b[j - 1]) {
        const trans = DP_BUFFER[(i - 2) * stride + j - 2] + 1;
        if (trans < val) val = trans;
      }
      DP_BUFFER[currRow + j] = val;
      if (val < rowMin) rowMin = val;
    }
    if (rowMin > maxDistance) return false;
  }

  return DP_BUFFER[la * stride + lb] <= maxDistance;
}
