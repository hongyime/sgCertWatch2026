...
function compact(value) {
  ...
  return unicodeSkeleton(norm).replace(/[^a-z0-9]/g, "");
}
...
const _dl0 = new Int32Array(256);
const _dl1 = new Int32Array(256);
const _dl2 = new Int32Array(256);

function damerauLevenshteinDistance(a, b, aStart = 0, aLen = a.length, maxDistance = 2) {
  const la = aLen;
  const lb = b.length;
  if (la === 0) return lb;
  if (lb === 0) return la;
  ...
  for (let i = 1; i <= la; i++) {
    r2[0] = i;
    let rowMin = i;
    const ai = a.charCodeAt(aStart + i - 1);
    for (let j = 1; j <= lb; j++) {
      ...
      if (i > 1 && j > 1 && ai === b.charCodeAt(j - 2) && a.charCodeAt(aStart + i - 2) === b.charCodeAt(j - 1)) {
        val = Math.min(val, r0[j - 2] + 1);
      }
      r2[j] = val;
      if (val < rowMin) rowMin = val;
    }
...
function minSubstringEditDistance(label, token, maxDistance, tokMask = null, tokUnique = 0) {
  if (!label || !token) return -1;
  const tokenLen = token.length;
  const labelLen = label.length;
  if (label.includes(token)) return 0;
  if (maxDistance <= 0) return -1;
  if (labelLen < tokenLen - maxDistance) return -1;

  // Fast bitmask reject: label must contain at least (uniqueTokens - maxDistance) unique chars of token
  const tMask = tokMask || getCharMask(token);
  const uCount = tokUnique || (popcount32(tMask.l) + popcount32(tMask.d));
  const minRequiredOverlap = Math.max(1, uCount - maxDistance);

  const labMask = getCharMask(label);
  const overlap = popcount32(labMask.l & tMask.l) + popcount32(labMask.d & tMask.d);
  if (overlap < minRequiredOverlap) return -1;

  if (Math.abs(labelLen - tokenLen) <= maxDistance) {
    const d = damerauLevenshteinDistance(label, token, 0, labelLen, maxDistance);
    if (d <= maxDistance) return d;
    if (labelLen === tokenLen) return -1;
  }
  ...
  for (let w = minWindow; w <= maxWindow; w++) {
    for (let i = 0; i <= labelLen - w; i++) {
      const d = damerauLevenshteinDistance(label, token, i, w, maxDistance);
  ...
  return bestDist <= maxDistance ? bestDist : -1;
}
...
function brandSignals(domain, data, passedHost = null, passedRegistrable = null) {
  ...
  const leetLabels = allLabels.flatMap(asciiHomoglyphs).map(compact);
  const combinedLabels = [...new Set([...allLabels, ...skelLabels, ...leetLabels])];

  const skelSubLabels = idn.skeleton ? subdomainLabels(idn.skeleton).map(compact) : [];
  const leetSubLabels = subLabels.flatMap(asciiHomoglyphs).map(compact);
  const combinedSubLabels = [...new Set([...subLabels, ...skelSubLabels, ...leetSubLabels])];
  const affixKeywords = data?.affixKeywords || (data?.keywords?.keywords || []).filter((k) => k.affix).map((k) => compact(k.token));
  let candidateLabelsWithAffix = null;
  let subCandidateLabelsWithAffix = null;

  for (const brand of data?.watchlist?.brands || []) {
    const compactTokens = brand._compactTokens || (brand.tokens || []).map(compact);
    const compactContext = brand._compactContextTokens || (brand.context_tokens || []).map(compact);
    const contextHit = !brand.require_context
      || compactContext.some((token) => hostCompact.includes(token) || skeletonCompact.includes(token));
    ...
    let candidateLabels = combinedLabels;
    let subCandidateLabels = combinedSubLabels;
    if (brand.allow_affix) {
      if (!candidateLabelsWithAffix) {
        candidateLabelsWithAffix = combinedLabels.flatMap((l) => decomposeLabel(l, affixKeywords));
        subCandidateLabelsWithAffix = combinedSubLabels.flatMap((l) => decomposeLabel(l, affixKeywords));
      }
      candidateLabels = candidateLabelsWithAffix;
      subCandidateLabels = subCandidateLabelsWithAffix;
    }

    for (let ti = 0; ti < (brand.tokens || []).length; ti++) {
      const token = brand.tokens[ti];
      const tokenCompact = compactTokens[ti];
      const exact = hostCompact.includes(tokenCompact) || skeletonCompact.includes(tokenCompact) || candidateLabels.includes(tokenCompact);
      let matchType = null;
      let matchDist = 0;
      ...
      if (exact) {
        ...
        const effectiveMaxDist = maxEditDistanceForLength(tokenCompact.length, brand.max_edit_distance ?? 1, brand.require_context);
        if (effectiveMaxDist > 0) {
          for (const label of candidateLabels) {
            // Dictionary word suppression for generic collisions (e.g. carousel vs carousell)
            if (brand.id === "carousell" && label.includes("carousel") && !label.includes("carousell")) {
              const hasExplicitContext = compactContext.some((ct) => hostCompact.includes(ct));
              if (!hasExplicitContext) continue;
            }

            const dist = minSubstringEditDistance(
              label,
              tokenCompact,
              effectiveMaxDist,
              brand._tokenMasks?.[ti],
              brand._tokenUniques?.[ti]
            );
            if (dist > 0) {
      ...
      const subEffectiveMaxDist = maxEditDistanceForLength(tokenCompact.length, brand.max_edit_distance ?? 1, brand.require_context);
      const subSquat = subCandidateLabels.some(
        (l) => l === tokenCompact || (subEffectiveMaxDist > 0 && minSubstringEditDistance(l, tokenCompact, subEffectiveMaxDist) > 0)
      );
...
function homoglyphSignals(domain, data, passedHost = null, passedRegistrable = null) {
  ...
  for (const label of labels) {
    if (!/[013458]|rn|vv|cl/.test(label)) continue;
    const leetCandidates = asciiHomoglyphs(label);
    if (leetCandidates.length === 0) continue;
...
function keywordSignals(domain, data) {
  ...
  for (const keyword of data?.keywords?.keywords || []) {
    const tokenCompact = keyword._compactToken || compact(keyword.token);
    if (hostCompact.includes(tokenCompact)) {
      hits.push({
        type: "kw",
        token: keyword.token,
...
function schemeSignals(domain, data, now = new Date()) {
  ...
  for (const scheme of data?.schemes?.schemes || []) {
    const compactTokens = scheme._compactTokens || (scheme.tokens || []).map(compact);
    const maxDist = scheme.max_edit_distance || 0;
    for (let ti = 0; ti < (scheme.tokens || []).length; ti++) {
      const token = scheme.tokens[ti];
      const tokenCompact = compactTokens[ti];
...
function scoreCertificate(entry, data, now = new Date()) {
  ...
  const maxCap = cfg.caps?.total ?? 150;
  const totalScore = Math.min(top.score + certPoints, maxCap);
