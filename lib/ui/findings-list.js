...
/**
 * Render a single finding table row `<tr>` for the desktop compact table
 * (>=1024px). Output is identical to the inline table row markup that
 * existed in app.js at commit 4f31167, before it was accidentally deleted
 * in commit 5dbf7483 while fixing an unrelated card-list regression.
 *
 * @param {object} finding
 * @param {string|null} selectedFindingId
 * @returns {string} HTML string
 */
export function renderFindingRow(finding, selectedFindingId, isSignedIn = false) {
  return `
    <tr data-finding-id="${escapeHtml(finding.id)}" aria-selected="${finding.id === selectedFindingId ? 'true' : 'false'}">
      <td class="col-domain">${escapeHtml(finding.registrable)}</td>
      <td>${escapeHtml((finding.matched_brands || []).join(', ') || '\u2014')}</td>
      <td><span class="severity ${escapeHtml(finding.severity)}">${escapeHtml(finding.severity)} ${escapeHtml(finding.priority_score ?? finding.score ?? 0)}</span></td>
      <td>${escapeHtml(intelHitCount(finding))}</td>
      <td>${escapeHtml(formatTime(finding.observed_at))}</td>
      <td class="col-review">${isSignedIn ? 'Open to review' : 'Sign in to review'}</td>
      <td><button type="button" class="btn-secondary" data-open-detail="${escapeHtml(finding.id)}">Details</button></td>
    </tr>`;
}

/**
 * Build the inner HTML (thead+tbody) for the desktop findings table.
 * The caller creates the `<table class="finding-list-table">` element and
 * assigns this string as its innerHTML.
 *
 * @param {object[]} findings
 * @param {string|null} selectedFindingId
 * @returns {string} HTML string
 */
export function renderFindingTableBody(findings, selectedFindingId, isSignedIn = false) {
  const head = '<thead><tr><th>Domain</th><th>Brand</th><th>Priority</th><th>Evidence</th><th>Observed</th><th>Review</th><th></th></tr></thead>';
  if (!findings.length) {
    return `${head}<tbody><tr><td colspan="7">No matching findings</td></tr></tbody>`;
  }
  const tbody = findings.map((f) => renderFindingRow(f, selectedFindingId, isSignedIn)).join('');
  return `${head}<tbody>${tbody}</tbody>`;
}
