...
export function encodeFilter(filter) {
  ...
  if (filter.severity !== "watch") params.set("s", filter.severity);
...
/**
 * Built-in preset views. These use actual brand IDs/categories from
 * watchlist.json via the "category:<id>" and "verdict:<id>" sentinel query
 * syntax recognised by matchesFindingQuery() in app.js — never a hardcoded
 * English word guess that may never appear in a finding's fields.
 * Labelled as "loaded-data views" until Task 8 enables true historical search.
 */
export const PRESET_VIEWS = [
  {
    name: "Government (loaded data)",
    filter: { query: "category:government", severity: "" },
  },
  {
    name: "Banks (loaded data)",
    filter: { query: "category:bank", severity: "" },
  },
  {
    name: "Provider-reported phishing (loaded data)",
    filter: { query: "verdict:phishing", severity: "" },
  },
];
