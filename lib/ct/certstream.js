...
async function runCertStreamSource(options = {}) {
  const WebSocketImpl = options.WebSocketImpl || WebSocket;
  const certStreamUrl = options.url || CERTSTREAM_URL;
  const sampleMs = Number(options.sampleMs || SAMPLE_MS);
  const openTimeoutMs = Number(options.openTimeoutMs || OPEN_TIMEOUT_MS);
  const maxMessages = Number(options.maxMessages || MAX_MESSAGES);
  ...
  return new Promise((resolve) => {
    let finished = false;
    const ws = new WebSocketImpl(certStreamUrl, {
    ...
    function finish(error = null) {
      ...
      const details = !error && scannedEntries === 0
        ? { state: "standby", note: "No certificate messages arrived during the short live-stream sample." }
        : {};
    ...
    const openTimer = setTimeout(() => finish(new Error("CertStream open timeout")), openTimeoutMs);
    const sampleTimer = setTimeout(() => finish(), sampleMs);
