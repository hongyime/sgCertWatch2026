...
export function verifyCheckpoint(checkpointText, publicKeyDerBase64) {
  ...
  try {
    ...
    const rootHashBuf = parsed.rootHash ? Buffer.from(parsed.rootHash, "base64") : null;

    for (const sig of parsed.signatures) {
      try {
        const sigBytes = Buffer.from(sig.signature, "base64");
        // 1. In signed-note format, first 4 bytes are key name hash, rest is signature
        ...
        // 2. RFC 6962 STH note format: [4-byte keyId][8-byte timestamp][2-byte alg][2-byte sigLen][DER signature]
        if (sigBytes.length >= 18 && rootHashBuf && rootHashBuf.length === 32) {
          const ts = sigBytes.readBigUInt64BE(4);
          const derSig = sigBytes.subarray(16);

          const sthData = Buffer.alloc(1 + 1 + 8 + 8 + 32);
          sthData.writeUInt8(0, 0); // version 0
          sthData.writeUInt8(1, 1); // signature_type tree_hash (1)
          sthData.writeBigUInt64BE(ts, 2);
          sthData.writeBigUInt64BE(BigInt(parsed.treeSize), 10);
          rootHashBuf.copy(sthData, 18);

          const isSthVerified = crypto.verify("sha256", sthData, pubKey, derSig);
          if (isSthVerified) {
            return { ok: true, parsed };
          }
        }
