import { httpResponseError, sourceResult } from "../common.js";
...
export async function pollStaticCtLog(log, cursorState = {}, startTime = Date.now()) {
  ...
  if (!res.ok) {
    throw httpResponseError(res, `Failed to fetch checkpoint: ${res.status} ${res.statusText} at ${checkpointUrl}`);
  }
  ...
  while (cursor < treeSize && tilesFetched < MAX_TILES_PER_LOG_PER_RUN) {
    ...
    try {
      ...
      const budgetStopped = remainingMs() <= 0 && ["TimeoutError", "AbortError"].includes(tileErr.name);
      if (!budgetStopped) errors.push({ tile_index: tileIndex, message: tileErr.message,
        status: tileErr.status, retry_at: tileErr.retry_at });
      break;
    }
  }
...
export async function runStaticCtSource({ staticLogs = [], state = {} }) {
  ...
  const cooldowns = Object.fromEntries(Object.entries(state.cooldowns || {}).filter(([, until]) => Date.parse(until) > startedAt));
  function rememberRateLimit(operator, error) {
    if (error.status !== 429) return;
    cooldowns[operator] = new Date(Math.max(Date.now() + 3600000, error.retry_at || 0,
      Date.parse(cooldowns[operator]) || 0)).toISOString();
  }
  ...
  for (let offset = 0; offset < staticLogs.length; offset += 1) {
    ...
    const operator = log.operator || logKey;
    if (Date.parse(cooldowns[operator]) > Date.now()) {
      cooldownSkipped += 1;
      errors.push({ log: log.description, status: 429, message: `Operator rate limit; retry after ${cooldowns[operator]}` });
      continue;
    }
    attemptedLogs += 1;
    try {
      ...
      for (const error of result.errors) rememberRateLimit(operator, error);
      ...
      rememberRateLimit(operator, err);
      errors.push({ log: log.description, message: err.message, status: err.status, retry_at: err.retry_at });
    }
  }
  ...
  return sourceResult({
    ...
    statePatch: {
      static_ct: {
        index: staticLogs.length ? (startIndex + visitedLogs) % staticLogs.length : 0,
        cursors,
        cooldowns
      }
    }
