...
function usableRfc6962Logs(logList) {
  return (logList.operators || [])
    ...
    .filter((log) => intervalIsCurrent(log.temporal_interval))
    .filter((log) => !log.url.includes("ct.example.com"))
    .filter((log) => !log.url.includes("cloudflare.com"))
    .filter((log) => String(log.operator).toLowerCase() !== "cloudflare")
...
async function runDirectCtSource({ state = {} }) {
  ...
  const errors = [];
  const entries = [];
  const logList = await fetchJson(LOG_LIST_URL, { timeoutMs: 10000 });
  const logs = usableRfc6962Logs(logList);
  if (!logs.length) {
    return sourceResult({
      source: "direct_ct",
      ...
      entries,
      scannedEntries: 0,
      errors: [{ message: "No readable RFC6962 logs found" }],
      details: { log_list_version: logList.version },
  ...
  return sourceResult({
    ...
    errors,
    details: {
      log_list_version: logList.version,
