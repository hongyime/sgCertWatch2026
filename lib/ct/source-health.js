...
export function summarizePrimaryHealth(runs) {
  const primary = runs.filter((run) => run.source === "direct_ct" || run.source === "static_ct");
  const available = primary.filter((run) => run.ok || run.scanned_entries > 0 || run.details?.successful_log_count > 0);
  return {
    ok: available.length > 0,
    health: primary.length === 2 && primary.every((run) => run.ok) ? "healthy" : available.length ? "partial" : "down",
    backup_degraded: runs.some((run) => !primary.includes(run) && !run.ok)
  };
}
