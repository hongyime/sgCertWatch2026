...
const NAME_CACHE_MAX = 200000;

export function normaliseName(raw) {
  if (typeof raw !== "string" || raw.length === 0) return null;
  const cached = NAME_CACHE.get(raw);
  if (cached !== undefined) return cached;
  const result = normaliseNameUncached(raw);
  if (NAME_CACHE.size >= NAME_CACHE_MAX) NAME_CACHE.clear();
  NAME_CACHE.set(raw, result);
  return result;
}

function normaliseNameUncached(raw) {
  ...
  if (!/[^a-z0-9.-]/.test(name)) {
    return { name, unicode: name, wildcard };
  }
  try {
