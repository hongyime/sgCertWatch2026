...
async function upsertCtRows(table, rows, { assertOwned = () => {} } = {}) {
  ...
  const saved = [];
  // Ingest only counts saved rows. Do not download the full evidence again.
  const identityColumn = table === "findings" ? "id" : "finding_id";
  for (let offset = 0; offset < rows.length; offset += 200) {
    assertOwned();
    const response = await dbFetch(`${SUPABASE_URL}/rest/v1/${table}?select=${identityColumn}`, {
      method: "POST",
...
/**
 * Bounded historical search using the workbench_search_findings RPC.
 * Uses anon credentials — never service-role.
 * @param {object} params  validated params from parseSearchParams
 * @param {object|null} cursorState  decoded cursor state or null for first page
 * @param {string} evaluatedAt  ISO timestamp for consistent pagination
 * @returns {Promise<{ findings: object[] }>}
 */
async function searchFindings(params, cursorState, evaluatedAt) {
  if (!configured("anon")) return { findings: [] };
  const signal = AbortSignal.timeout(8500);
  const body = {
    p_q:            params.q            || "",
    p_severity:     params.severity     || "",
    p_brand_id:     params.brand_id     || "",
    p_from_at:      params.from_at      || "",
    p_to_at:        params.to_at        || "",
    p_verdict:      params.verdict      || "",
    p_source:       params.source       || "",
    p_priority_min: params.priority_min ?? null,
    p_priority_max: params.priority_max ?? null,
    p_sort:         params.sort         || "observed",
    p_limit:        params.limit        || 50,
    p_evaluated_at: evaluatedAt,
  };
  if (cursorState) {
    body.p_after_id  = cursorState.last_id;
    body.p_after_obs = cursorState.last_observed_at;
    body.p_after_pri = cursorState.last_priority;
  }
  const response = await dbFetch(`${SUPABASE_URL}/rest/v1/rpc/workbench_search_findings`, {
    method: "POST",
    headers: anonHeaders(),
  ...
  if (!response.ok) {
    throw new Error(`Supabase search query failed: ${response.status} ${await response.text()}`);
  }
  const result = await response.json();
  const findings = Array.isArray(result?.findings) ? result.findings : [];
  return { findings, evaluated_at: result?.evaluated_at ?? evaluatedAt };
}
