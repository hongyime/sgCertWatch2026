...
async function upsertFindings(findings) {
  ...
  const response = await dbFetch(`${SUPABASE_URL}/rest/v1/findings`, {
    method: "POST",
...
async function upsertFindingSources(sources) {
  ...
  const response = await dbFetch(`${SUPABASE_URL}/rest/v1/finding_sources`, {
    method: "POST",
    headers: serviceHeaders({ Prefer: "resolution=merge-duplicates,return=representation" }),
...
async function tryAcquireRunLock(lockName = "ct_poll_run", timeoutSeconds = 300) {
  ...
  try {
    const existing = await getServiceState(`lock_${lockName}`);
    const now = Date.now();
