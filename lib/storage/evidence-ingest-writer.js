...
export function evidenceIngestWriter({ url, serviceKey, fetchImpl, objects, lease }) {
  ...
  return {
    ...
    async upsertFindingSources(rows, options) {
      // Existing source updater is per finding; batch/cost and every legacy
      // source reader remain cutover gates, rather than a production fallback.
      const groups = new Map();
      for (const row of rows) { const group = groups.get(row.finding_id) ?? []; group.push(row); groups.set(row.finding_id, group); }
      for (const [id, group] of groups) for (let start = 0; start < group.length; start += 200) {
        await repository.upsertSourceRows(id, group.slice(start, start + 200).map(row => Buffer.from(JSON.stringify(row))), options);
      }
      return rows.map(row => ({ finding_id: row.finding_id }));
    }
  };
}
