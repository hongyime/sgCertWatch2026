...
function sourceIdentity(bytes, findingId, complete = true) {
  const row = originalRow(bytes);
  if (row.finding_id !== findingId) throw new Error("Source belongs to another finding");
  if (complete && (!Object.hasOwn(row, "details") || typeof row.observed_at !== "string" || !row.observed_at
  ...
  // PostgreSQL's source key permits empty strings; they are real values, not
  // missing fields. Keep them distinct while retaining the transport budget.
  for (const value of [row.source, row.source_ref]) {
    if (typeof value !== "string" || Buffer.byteLength(value) > 8192) throw new Error("Invalid source identity");
  }
...
export class EvidenceRepository {
  ...
  async publish(findingId, { finding, sources }, expectedRevision, { assertOwned = () => {} } = {}) {
    ...
    assertOwned(); await this.privateManifests.assertLease(); assertOwned();
  ...
  // Complete snapshots are used by migration/backfill callers.
  async upsertSources(findingId, incoming, options = {}) {
    return this.updateSources(findingId, incoming, options);
  }

  // Production-shaped partial source rows need PostgreSQL's own defaults/type
  // conversion before they can become complete byte-preserving snapshots.
  async upsertSourceRows(findingId, incoming, options = {}) {
    identity(findingId);
    if (!Array.isArray(incoming) || !incoming.length || incoming.length > 200) throw new Error("Invalid source upsert batch");
    const keys = incoming.map(row => sourceIdentity(row, findingId, false));
    if (new Set(keys).size !== keys.length) throw new Error("Duplicate source upsert identity");
    if (typeof this.privateManifests.prepareRows !== "function") throw new Error("Source normalization is required");
    return this.updateSources(findingId, incoming, options, async current => {
      const normalized = await this.privateManifests.prepareRows("source", current.sources, incoming);
      if (!Array.isArray(normalized) || normalized.length !== keys.length
          || normalized.some((row, index) => sourceIdentity(row, findingId) !== keys[index])) {
        throw new Error("Invalid source normalization acknowledgement");
      }
      return normalized;
    });
  }

  async updateSources(findingId, incoming, { maxConflicts = 3, assertOwned = () => {} } = {}, prepare = null) {
    identity(findingId);
    ...
    for (let attempt = 0; attempt <= maxConflicts; attempt++) {
      assertOwned();
      await this.privateManifests.assertLease(); assertOwned();
      ...
      const normalized = prepare ? await prepare(current) : incoming;
      assertOwned();
      const sources = mergeSourceRows(current.sources, normalized, findingId);
      if (sources.length === current.sources.length && sources.every((row, index) => row.equals(current.sources[index]))) {
        assertOwned(); await this.privateManifests.assertLease(); assertOwned(); return current.manifest;
      }
