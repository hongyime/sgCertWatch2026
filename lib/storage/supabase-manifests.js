...
function fromRow(rows, id, privateRead) {
  ...
  validatePointer(row.finding_pointer);
  if (privateRead && row.sources_pointer !== null) validatePointer(row.sources_pointer);
  ...
  return { id, revision, suppressed, finding: row.finding_pointer,
    ...(privateRead ? { sources: row.sources_pointer } : {}) };
}
