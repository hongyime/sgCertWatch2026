...
2026-09-12 experimental lossless storage adapter: lib/storage contains byte-preserving frames, immutable private-object transport, anonymous/public and service/private manifest clients, and optimistic concurrent publication. The public batch checks anonymous/RLS manifests once and downloads each packed object once, returning only requested authorized rows. Sources use complete identity tuples and original JSON bytes, retaining large numbers, timestamps and references. Source updates reuse unchanged finding pointers; identical retries publish nothing. All 21 offline unit cases and eight PostgreSQL test results (seven contract cases plus their suite) pass locally on Node 26.5 / PostgreSQL 17.11. Hosted Node 20/24 and PostgreSQL 17.11 checks are pending. The standalone local lab is stopped, with no shared Docker startup. This is review-stage preparation: production still uses lib/supabase.js, api/triage.js, capture and intel RPCs against the existing findings/source tables. Do not apply supabase/experimental/evidence-manifests.sql to production yet. Full read/write integration, original created_at/upsert semantics, database lease fencing, actual Storage authorization, full sizing/packing, orphan lifecycle, migration peak space and rollback remain required. The adapter is not a proven free-tier solution and no production records or collector schedules changed.

2026-09-12 checkpoint v51: draft PR #14 is checked at 8b93a1ea318328b21612bc5e5d4416591ffedc39. All 21 offline tests and seven PostgreSQL contract cases pass on both hosted Node 20/24. CI container identity now uses the exact Actions service and step context; all applicable security/validation checks and Vercel preview pass. Nine actual private Storage tests pass with 355 synthetic bytes; both objects and the temporary bucket are removed, zero Storage objects/buckets remain, and anon credentials passed a separate zero-row positive control. Application records, production schema and collection are unchanged. Production stays READY at 46958b60a4e3c9e760905bc2a0cd8b44b7527be3. The local PostgreSQL lab is stopped. PostPlan v51 is byte-verified at https://aoo181uudk96.postplan.dev. Evidence: certwatch-storage-adapter-closeout.json in the portfolio audit. Next: full reader/writer/timestamp and lease contracts, compact metadata/full sizing, read amplification/ranges, orphan lifecycle, migration space, growth and rollback. This draft is not a production migration or proof of free-tier fit. Preserve all original worktree edits and SG SHIOK/shared Docker state.


2026-09-12 compact manifest continuation: replacing experimental JSONB pointers with fixed 40-byte binary pointers (full SHA-256 plus bounded offset/length), retaining original finding IDs, public/private grants, API pointer shape and CAS behavior. All 23 offline cases pass; real PostgreSQL and capacity checks are next. A live full-width/count aggregate timed out; catalogue row estimates are explicitly not exact counts. Production experimental schema and runtime remain unchanged. Preserve the checked draft and all previous work.


2026-09-12 compact/batch source update: fixed 40-byte pointers preserve all digest bits and original identity strings. Bounded publishBatch packs 200 complete snapshots into shared finding/source objects and uses one service-only RPC with sorted lock order, explicit revision conflicts and whole-RPC rollback on validation/visibility failure. The fixture changes 400 uploads to two and 200 publication calls to one; a 100-finding page reads one object. All 28 offline tests and 13 PostgreSQL contract cases (14 test results) pass locally. The 300,000-row metadata fixture measures 72,949,760 bytes versus 129,744,896 bytes, with no pointer mismatches; an interrupted measurement was resumed from preserved rows. Final 2,000-row/resume checks also pass. Hosted source validation remains pending. Production is unchanged: full reader/writer/timestamp integration, DB lease fencing, full capacity and Storage metadata, object lifecycle, egress, growth and rollback still prevent cutover. No Docker, SG SHIOK or collector changes.

2026-09-12 compact/batch checkpoint complete: PR #14 passes on Node 20/24 at f2de10f929fcd9e0724e379b8d226a2e9a311350, including 28 offline checks, 13 PostgreSQL contract cases and metadata/resume checks. Preview is READY; production stays READY at 46958b60a4e3c9e760905bc2a0cd8b44b7527be3 and experimental schema is absent. The 300,000-row fixture saves 43.77% of manifest table/primary-key bytes, not whole-project storage. Actual adapter measurements show grouped objects increase one-finding reads from 109 to 22,090 payload bytes, and 100-finding reads from 10,990 to 22,090; uploaded bytes are unchanged. Full contract, database lease fencing, lifecycle/capacity, weighted egress/range integrity, growth and rollback remain open. Original edits are preserved and the owned PostgreSQL lab is stopped. No records, collector schedules, Docker or SG SHIOK changed. PostPlan v54 is published and byte-verified at https://aoo181uudk96.postplan.dev. Evidence: certwatch-manifest-capacity-review.json and certwatch-manifest-capacity-closeout.json in the portfolio audit.

2026-09-12 row-contract continuation: map every production finding/source reader and writer; reproduce PostgreSQL partial-upsert/default/timestamp behavior; add database-backed canonicalization and use it for source upserts with optimistic retry; validate precision, omitted/null fields and constraints against native table writes; then publish only the checked draft and report. Production remains unchanged. Reader/search/capture/intel/finding-writer cutover, database lease fencing, full capacity/lifecycle/egress and migration rollback remain separate open gates. Original worktree handoff edits and all records are preserved.

2026-09-13 SGT row-contract implementation: service-only prepare_evidence_rows preserves current PostgreSQL defaults, partial-column updates, timestamp precision and JSONB numeric digits as text. upsertSourceRows re-normalizes after CAS conflicts, retains original created_at and reuses unchanged bytes on retry. The 10,000-source fixture originally exceeded its eight-second statement deadline; one aggregate replaced repeated map rebuilding and completed normalization in 785 ms. Empty source names/references are now retained as valid distinct SQL keys. Local final tests and hosted draft checks are next. Finding IDs still require nonempty bounded strings; arbitrary future defaults/schema changes and complete production reader/writer/lease contracts remain unverified. Production is unchanged.

2026-09-13 SGT row-contract checkpoint: draft PR #14 is checked at e0f73d431edaf1678b188090d1819d586cbe5044. Hosted Node 20/24 pass 33 offline cases, 17 PostgreSQL integration cases and 19 native row-differential cases, plus metadata/resume checks. Applicable PR checks and preview pass; launch readiness is skipped for the draft. Production remains READY at 46958b60a4e3c9e760905bc2a0cd8b44b7527be3 and all three experimental schema objects are absent. The latest eight-project read-only size snapshot records SGCertWatch at 1,428.5 MB across databases (00:10 SGT); capacity remains unresolved. All 26 latest Vercel deployments are READY, with web undeployed. PostPlan v58 is published and byte-verified at https://aoo181uudk96.postplan.dev. Evidence: certwatch-row-contract-review.json and certwatch-row-contract-map.json in the portfolio audit. These handoff notes are the only uncommitted worktree edits. The standalone synthetic database is stopped, original checkout edits are preserved, and shared Docker/SG SHIOK and collection are unchanged. Next: rotate through the broader queue and complete production integration, DB lease fencing, oversized records, full capacity/lifecycle/egress, growth and migration rollback. The portfolio goal remains active.

2026-09-13 SGT HTTP contract continuation: the production OpenAPI reports PostgREST 14.5; its anonymous zero-row control succeeds. A checksum-verified standalone 14.5 server and PostgreSQL 17.11 fixture reproduce one draft bug: heterogeneous bulk column sets were accepted although native PostgREST rejects them. The normalizer now requires identical column sets while permitting different JSON property order. All 19 real HTTP cases, 33 offline cases, 19 native SQL row cases and 17 PostgreSQL integration cases pass locally. The HTTP suite uses synthetic JWTs, a dedicated authenticator and an owned loopback server; private object bytes are in-memory fixtures. Node 20/24 CI now includes the pinned HTTP server. Hosted checks are pending. Both local servers are stopped; production schema, records, collector and shared Docker/SG SHIOK remain unchanged. Full reader/writer integration, database lease fencing, oversized rows, capacity/lifecycle/egress/growth and migration rollback remain cutover gates.

2026-09-13 SGT HTTP checkpoint complete: draft PR #14 at 904c358ed7ee49a94cf9bd204f96b28f3b74804e passes all applicable checks, including both Node 20/24 jobs with 33 offline, 17 PostgreSQL integration, 19 native SQL and 19 real PostgREST HTTP cases plus metadata/resume checks. Preview is READY. Production remains READY at 46958b60a4e3c9e760905bc2a0cd8b44b7527be3; five experimental table/function catalog checks are absent. All 26 deployed portfolio projects have READY latest and production deployments; web remains undeployed. At 04:32 SGT, SGCertWatch measures 1,463.3 MB across databases. PostPlan v65 is published and byte-verified at https://aoo181uudk96.postplan.dev#certwatch-http-draft. Evidence: certwatch-http-review.json and certwatch-http-closeout.json in the portfolio audit. Original checkout edits are untouched, the owned PostgreSQL/PostgREST servers are stopped, and no collector, records, shared Docker or SG SHIOK changes occurred. Full integration, DB lease fencing, oversized records, full capacity/lifecycle/egress/growth and migration rollback remain open. Rotate to Prawn Home CodeQL coverage while retaining the full portfolio scope.

2026-09-13 SGT lease continuation: experimental manifest CAS checks revisions but has no database worker-ownership fence. Reproduce stale publication after lease takeover, bind writers to a fresh lease owner, validate ownership through the SQL transaction and block unfenced direct manifest DML, then run concurrency and HTTP checks. Draft PR #14 stays unmerged; production records, schema and collection are unchanged. Detailed five-task handoff: .agents/handoffs/certwatch-lease-20260913.json.

2026-09-13 SGT lease implementation: the stale-worker regression reproduced an unfenced write after takeover. Experimental writers now bind a fresh lease owner at construction; service-only publication RPCs hold the run-lock row through commit, recheck wall-clock expiry after waits, and reject lost leases with HTTP 409. Direct worker manifest DML and old unfenced RPC overloads are absent. No-op source retries recheck ownership. Local 36 offline, 28 PostgreSQL and 22 real HTTP cases pass, including upload/commit races, batch rollback, two-second lock timeout and search-path isolation. Hosted Node 20/24 is pending. Both owned synthetic servers are stopped; original checkout, credentials and production are preserved. Publication adds one preflight request per single or 200-snapshot batch; full integration/capacity/lifecycle/egress/growth/rollback remain open.

2026-09-13 SGT lease checkpoint complete: draft PR #14 at df907674b2f59bd0eac95196b328209141fa500a passes 36 offline, 28 PostgreSQL, 19 native-row and 22 real HTTP cases on both Node 20/24, plus metadata/resume checks. All 16 applicable PR checks pass; draft launch readiness is skipped and preview is READY. Production remains READY at 46958b60a4e3c9e760905bc2a0cd8b44b7527be3; eight experimental catalog checks are absent. Original checkout files, edits and environment files are unchanged. Both owned fixture servers are stopped; no production records, collector, shared Docker or SG SHIOK changes. At 07:58 SGT SGCertWatch totals 1,512.7 MB across databases, still above the free allowance. At 07:56 SGT all 26 deployed Vercel projects have READY latest and production deployments; web is undeployed. PostPlan v68 is published and byte-verified at https://aoo181uudk96.postplan.dev#certwatch-lease-draft. Evidence: certwatch-lease-review.json and certwatch-lease-closeout.json in the portfolio audit. Full integration, oversized rows, capacity/lifecycle/egress/growth and rollback remain open. Rotate to Pokemoncards persistence and the remaining portfolio queue. Completion notes are the only uncommitted owned-worktree changes; no additional release is implied.

2026-09-13 SGT finding-writer continuation: integrate a lease-bound experimental ingest writer with bounded finding batches, lossless PostgreSQL row text and atomic row/manifest publication. Preserve pre-existing sightings during first publication; detect concurrent relational edits before acknowledgment. Production remains unchanged and draft PR #14 stays unmerged. Five-task handoff: .agents/handoffs/certwatch-finding-writer-20260913.json.

2026-09-13 SGT finding-writer implementation: one bounded context read, native PostgreSQL normalization and atomic row/manifest commit integrate with an explicitly injected writer factory bound to the actual acquired ingest lease. First publication preserves old source rows; source changes and relational/manifest races cause bounded retry. Review reproduced a stale-body no-op after a legacy edit; grouped object verification now refreshes that body without losing precision. Local HTTP tests and concurrency extensions are in progress; production activation remains absent. The previous stopped local fixture was missing pg_notify; a new isolated synthetic cluster was initialized without editing prior fixture data.

2026-09-13 SGT finding-writer local validation: all 37 real HTTP tests pass on Node 24.21.0/PostgreSQL 17.11/PostgREST 14.5, including actual source-row lock waiting, lease takeover during upload, full-batch rollback and real ingest cursor failure cases. Existing unit and reliability suites pass. The stale existing-body regression was reproduced before repair. The 200-new-finding fixture uses one upload/verification plus one context, normalization, commit and preflight RPC each; identical retries verify one shared object without uploads/revision writes. All 182 original tracked files and environment hashes are preserved. Both owned fixture servers are stopped. Hosted draft checks are next.

2026-09-13 SGT finding-writer hosted review: draft PR #14 at 028ed384ace8b93d2ba765f8690b6c548a239691 passes all 16 applicable checks; draft launch readiness is skipped. Both hosted Node 20/24 pass 36 offline, 28 PostgreSQL, 19 native-row and 36 HTTP cases plus metadata/resume checks. Preview is READY; production remains 46958b60a4e3c9e760905bc2a0cd8b44b7527be3 with ten experimental catalog objects absent. All 26 deployed portfolio homepages return HTTP 200 without Vercel deployment-error headers; web remains undeployed. Original files, edits and environment hashes are preserved. PostPlan 75 desktop/mobile verification and publication are pending. Next rotation: UnifiedCollector dependency-ordered migrations without editing applied SQL checksums; preserve its runtime-location question.

2026-09-13 SGT finding-writer checkpoint complete: PostPlan 75 is published and byte-verified at https://aoo181uudk96.postplan.dev#certwatch-finding-writer-draft. Draft PR #14 at 028ed384ace8b93d2ba765f8690b6c548a239691 is checked and remains unmerged. Original checkout files/edits/environments and production are preserved; both owned fixture servers are stopped. Only completion notes are uncommitted. SGCertWatch remains above the free database allowance; full source batching, reader/writer/outbox integration, capacity/lifecycle/egress/growth and migration rollback remain open. Next: UnifiedCollector migration dependencies and full-suite gates, retaining its pending runtime-location question. Evidence: certwatch-finding-review.json and certwatch-finding-closeout.json in the portfolio audit. The entire portfolio goal remains active.

2026-09-13 SGT source-writer continuation: integrating batched source-sighting publication with native relational upserts and lease/revision/row conflict checks. Real isolated HTTP regression cases are running against the previous draft first. Existing production and original worktree are preserved. Full capacity, lifecycle, reader/writer coverage and rollback remain open. Resume details: .agents/handoffs/certwatch-source-writer-20260913.json.

2026-09-13 SGT source-writer local validation: all 45 real HTTP cases (46 total results) pass, including 200-row batching, precision, native edits/lock waits, concurrent publishers, full-RPC rollback, lease takeover and unchanged ingest cursors after SQL materialization failure. Existing unit/reliability and 36 offline cases pass. The isolated servers are stopped. Native source rows and pointers now commit together; object-only older draft sightings are rejected pending reconciliation instead of discarded. Hosted draft checks and portfolio update are next; production and full storage cutover gates remain unchanged.

2026-09-13 SGT source-writer final local check: 46 HTTP cases (47 total results) pass. Added unchanged physical-row fingerprints and stale source-object refresh after a native edit. The parent pointer stays unchanged and native rows are not rewritten when only the old object needs repair. Source draft commit/hosted checks are next.
