...
Current task: Organization-targeted scheduler token is installed and dispatch is enabled. Real observation started 2026-09-09T00:45:44.188Z; verify the first due Cloudflare-origin scan, then retain the actual 24-hour observation gate. Telegram remains removed.

Current evidence and next steps:
- Replacement token is stored only in ignored `.env.scheduler`; dispatch authorization probe returns HTTP422 for the deliberately nonexistent ref. Worker GITHUB_TOKEN is replaced, other secrets retained, and deployment `732da869-4e63-4cbb-9c2c-ebf727c1fc75` enables dispatch. Real enabled tick at 00:45:44Z is healthy/error-free and starts dashboard soak evidence. Both scans correctly skip as recent_actual_start; fallback CT `34295622138` started 00:35. First CT opportunity is the 00:50 tick; await real dispatch and persistence. Do not expose the token or replace it with broad machine credentials.
...
- Cloudflare is deployed at `sgcertwatch-scheduler.hongyime.workers.dev`, paused with the supplied scoped token, random protected status credential and Supabase anon heartbeat. Live checks: public status 401, mutation 404, authenticated config dashboard/none, enabled=false, no configuration errors. Real cron ticks observe both workflows as dispatch_paused and recorded a dashboard_only CT incident with zero delivery attempts. No live external dispatch or soak is claimed. Reviewer fixed the initial-start/success gap bound with 27 passing tests, including actual engine simulations.
- CI `34294413421` passes actual PostgreSQL 17 outbox/lease tests, all runtime suites and Chromium desktop/mobile flows. CodeQL, Semgrep, TruffleHog and LFS checks pass. Both production aliases pass live Domains, details/evidence, JSON/CSV exports, Watchlist, Review, Monitor and favicon checks at 1440/390px; screenshots including the mobile first viewport were inspected.
- Intel `34294443227` succeeded on the new SHA and retained provider schedules without unnecessary requests. CT `34294438895` succeeded at 00:23:32Z: 39,257 entries checked, 1,736 findings and 1,752 sightings persisted. crt.sh returned 11 entries and direct CT succeeded; static logs retained genuine TrustAsia timeouts and Geomys/IPng rate limits, so coverage remains partial. Monitor changed from stale/running to fresh/completed and has no obsolete notification status. Read-only production SQL confirms zero total notification jobs and zero jobs since the retired scan started. All owned verification processes finished and reviewers are closed.
