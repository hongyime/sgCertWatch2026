...
- Next: user edits the same supplied token to select this repository and grant Actions read/write. Recheck authorization once; only after success activate with `--var DISPATCH_ENABLED:true`. Verify actual Cloudflare-origin scans and a measured 24-hour soak. No new token needs to be pasted in chat; ignored setup already retains the supplied token and protected status credential.
