...
- 2026-08-22: Commit 11C (Re-mine eval corpus from real observed certificates and fix harness reporting): Scanned 122,763 real certificates across 4 Let's Encrypt Static CT logs (`Sycamore2026h2`, `Willow2026h2`, `Sycamore2027h1`, `Willow2027h1`) to mine 2,606 hard negatives in the 25–69 ambiguity band, each carrying complete 4-field provenance (`log_id`, `tree_index`, `cert_sha256`, `observed_at`). Segregated legacy synthetic fixtures into `fixtures/corpus/constructed.jsonl` (`constructed: true`) and excluded them from headline benchmark metrics. Reported adversarial fixtures (60 items) on a separate line. Fixed denominator reporting in `scripts/eval.js` ($N = 2,876$, 160 positives, 2,716 negatives) with mathematical self-consistency assertions. Baseline metrics at alert threshold 70: Precision 96.97% (TP=128, FP=4, TN=2,712, FN=32), Recall 80.00%, F1 87.67%. Adversarial pass rate: 26/60 (43.3%).
