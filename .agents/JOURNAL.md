...
- 2026-08-22: Commit 11B (Repair dead scoring config and substring-window fuzzy brand matching): (1) Repaired four dead config keys: `scoring.combo_with_brand` (+25), `scoring.combo_with_scheme` (+25), `schemes.json` `seasonal_boost` (+15 with `active_window` check), and `scheme.max_edit_distance`. (2) Fixed `brand:fuzzy` asymmetry bug where whole-label exact equality defeated multi-token and affixed domains; implemented Damerau-Levenshtein transposition and substring-window matching (`substringEditDistanceAtMost`), enabling `dsb-secure-login-verify.xyz` to fire `brand:fuzzy` (+25) and `tld:mismatch` (+15). (3) Solved score ceiling contradiction allowing pure scheme lures (`cdcv0ucher.xyz`, `cdcvouchr.top`, `cdcvoucher-claim-portal-2026.xyz`) to reach alert threshold (>= 70) without requiring brand hits. (4) Enforced total score cap in `lib/scoring.js` and added validator check in `scripts/validate_data.py` blocking unreferenced scoring config keys in `lib/`. Updated baseline eval F1 to 64.21%, adversarial pass rate 26/60.
